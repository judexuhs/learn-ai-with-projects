...
export function ProjectGuide({ project }: { project: Project }) {
  ...
  useEffect(() => {
    try {
      const stored = JSON.parse(
        localStorage.getItem(progressKey(project.slug)) ?? '[]',
      );
      const valid = normalizeProgress(stored, stepIds);
      setCompleted(valid);
      setSelected(nextStepId(valid, stepIds) ?? stepIds[0]);
      setTodoProgress(
        JSON.parse(localStorage.getItem(todoProgressKey(project.slug)) ?? '{}'),
      );
    } catch {
      setCompleted([]);
      setTodoProgress({});
  ...
  const current =
    project.steps.find((step) => step.id === selected) ?? project.steps[0];
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[240px_minmax(0,760px)] lg:justify-center lg:items-start">
      <aside className="lg:sticky lg:top-24">
        ...
        <label
          className="mt-4 block text-xs font-medium text-slate-500 lg:hidden"
          htmlFor="project-step-select"
        >
          选择步骤
        </label>
        <select
      ...
      <article key={current.id} className="min-w-0">
        <header className="border-b border-black/[.08] pb-8">
          <p className="text-sm font-medium text-sky-700">
            第 {currentIndex + 1} 步，共 {project.steps.length} 步
          </p>
          ...
          <div className="mt-6 rounded-lg bg-sky-50 px-4 py-4 sm:px-5">
            ...
            <p className="mt-1.5 text-base font-medium leading-7 text-sky-950">
              {current.outcome}
            </p>
          </div>
        </header>
        ...
        <section className="py-8">
          <div className="flex items-end justify-between gap-4">
            ...
            <span className="shrink-0 text-xs tabular-nums text-slate-400">
              {currentTodoProgress.length} / {todos.length}
            </span>
          </div>
                ...
                >
                  <label className="flex cursor-pointer items-start gap-3.5">
                    <input
                      ...
                      checked={checked}
                      onChange={(event) =>
                        updateTodo(current.id, item.title, event.target.checked)
                      }
                    ...
                    <span className="min-w-0">
                      ...
                      {item.recommendation && (
                        <span className="mt-2 block text-xs leading-5 text-sky-700">
          ...
          <ul className="mt-3 space-y-2.5 text-sm leading-6 text-slate-600">
            {current.checks.map((item) => (
              <li key={item} className="flex gap-2.5">
                <Check className="mt-1 size-4 shrink-0 text-sky-700" />
                {item}
              </li>
            ))}
          </ul>
        ...
        <footer className="flex flex-col gap-4 pt-8 sm:flex-row sm:items-center sm:justify-between">
          ...
          {nextStep ? (
            <Button
              onClick={() => setSelected(nextStep.id)}
