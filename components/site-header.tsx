    ...
    <header className="sticky top-0 z-20 border-b border-black/[.08] bg-white/92 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between border-x border-black/[.06] px-5 md:px-8">
        ...
        <details className="group relative md:hidden">
          <summary
            className="flex size-9 cursor-pointer list-none items-center justify-center rounded-md border border-black/10 text-neutral-700 [&::-webkit-details-marker]:hidden"
            aria-label="打开导航菜单"
          >
            <Menu className="size-4" />
          </summary>
          <nav
            className="absolute top-12 right-0 z-30 w-56 rounded-lg border border-black/10 bg-white p-2 shadow-[0_16px_32px_rgba(0,0,0,.08)]"
            aria-label="移动端导航"
          >
            {nav.map(([label, href]) => (
              <Link
                key={href}
                href={href}
                className="block rounded-md px-3 py-2.5 text-sm text-neutral-700 hover:bg-black/[.04]"
              >
                {label}
              </Link>
            ))}
            <Link
              href="/start"
              className="mt-1 block rounded-md bg-[#1d1d1f] px-3 py-2.5 text-center text-sm font-medium text-white"
            >
              从这里开始
            </Link>
          </nav>
