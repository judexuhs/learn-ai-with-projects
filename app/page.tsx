...
import { ArrowRight, BookOpen, Check, FlaskConical, Layers3, Wrench } from 'lucide-react';
  ...
  return <main className="min-h-dvh bg-white text-[#1d1d1f]">
    ...
    <section className="mx-auto grid max-w-6xl gap-12 border-x border-black/[.06] px-5 pt-16 pb-16 md:grid-cols-[1.08fr_.92fr] md:px-8 md:pt-20 md:pb-20">
      ...
      <div className="self-center rounded-xl border border-black/[.08] bg-[#fafafa] p-5 sm:p-6">
        ...
        <ol className="mt-5 space-y-5">{learningSteps.map((step, index) => <li key={step.title} className="grid grid-cols-[32px_1fr] gap-3">
          <span className="flex size-8 items-center justify-center rounded-md border border-black/[.08] bg-white font-mono text-xs text-neutral-500">{index + 1}</span>
          <div><h2 className="text-sm font-semibold">{step.title}</h2><p className="mt-1 text-sm leading-6 text-neutral-500">{step.description}</p></div>
        </li>)}</ol>
      </div>
    </section>
    ...
    <section className="border-y border-black/[.08] bg-[#fafafa]">
      <div className="mx-auto max-w-6xl border-x border-black/[.06] px-5 py-14 md:px-8 md:py-16">
        <div className="max-w-2xl">
          <h2 className="text-3xl font-semibold tracking-[-0.04em]">已经有明确目标？</h2>
          <p className="mt-4 leading-7 text-neutral-500">只有在你已经知道想做什么时，才需要选择下面两条路径。</p>
        </div>

        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {advancedPaths.map((path) => { const Icon = path.icon; return <article key={path.href} className="rounded-xl border border-black/[.08] bg-white p-5 sm:p-6">
            <span className="flex size-9 items-center justify-center rounded-md bg-[hsl(212_100%_97%)] text-[hsl(212_100%_41%)]"><Icon className="size-4" /></span>
            <h3 className="mt-5 text-xl font-semibold tracking-[-0.025em]">{path.title}</h3>
            <p className="mt-2 max-w-md text-sm leading-6 text-neutral-500">{path.description}</p>
            <Link href={path.href} className={cn(buttonVariants({ variant: 'outline' }), 'mt-6')}>{path.action}<ArrowRight /></Link>
          </article>; })}
        </div>

        <Link href="/methods" className="mt-6 inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-black">查看三条路径的完整对比<ArrowRight className="size-4" /></Link>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-6xl border-x border-black/[.06] px-5 py-14 md:px-8 md:py-16">
      <div className="grid gap-10 md:grid-cols-[1fr_360px] md:items-center">
        <div>
          <p className="flex items-center gap-2 text-sm font-medium text-[hsl(212_100%_41%)]"><FlaskConical className="size-4" />不想先学，也可以直接做</p>
          <h2 className="mt-4 text-3xl font-semibold tracking-[-0.04em]">先做一个不用 AI 的答案之书</h2>
          <p className="mt-4 max-w-2xl leading-7 text-neutral-500">从一个 HTML 文件开始。你会先经历定义、生成、运行和修改，再决定是否继续学习 AI 调用。</p>
        </div>
        <div className="rounded-xl border border-black/[.08] bg-white p-5 shadow-[0_4px_18px_rgba(0,0,0,.04)]">
          ...
          <ul className="mt-4 space-y-2 text-sm text-neutral-500">{['不需要 API', '约 90 分钟', '最终得到可分享网页'].map((item) => <li key={item} className="flex items-center gap-2"><Check className="size-4 text-[hsl(212_100%_41%)]" />{item}</li>)}</ul>
