...
export async function generateMetadata({
  ...
  return path ? { title: path.title, description: path.outcome } : {};
}
    ...
    <main className="min-h-dvh bg-white text-[#1d1d1f]">
      ...
      <section className="border-b border-black/[.08] bg-[#fafafa]">
        <div className="mx-auto max-w-6xl border-x border-black/[.06] px-5 py-12 md:px-8 md:py-16">
          ...
          <div className="mt-10 grid gap-9 md:grid-cols-[1fr_300px] md:items-end">
            <div>
            ...
            <div className="rounded-xl border border-black/[.08] bg-white p-5">
              <p className="text-xs text-neutral-400">适合谁</p>
              <p className="mt-2 text-sm leading-6 text-neutral-700">
                {path.forWhom}
              </p>
            </div>
          </div>
          <div className="mt-10 flex gap-3 rounded-xl border border-[hsl(212_100%_88%)] bg-[hsl(212_100%_97%)] p-5">
            <Check className="mt-1 size-4 shrink-0 text-[hsl(212_100%_41%)]" />
            <p className="text-sm leading-6 text-neutral-700">
              <strong>完成后的结果：</strong>
              {path.outcome}
            </p>
          </div>
        </div>
            ...
            return (
              <li key={step.topic}>
                <Link
                  ...
                  className="group grid gap-5 rounded-xl border border-black/[.08] p-5 transition hover:border-black/20 hover:bg-black/[.012] md:grid-cols-[44px_220px_1fr_150px] md:items-center md:p-6"
                >
                  <span className="flex size-9 items-center justify-center rounded-md bg-[#fafafa] font-mono text-xs text-neutral-400">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  <div>
                    <p className="text-xs text-[hsl(212_100%_41%)]">
                      {topic.category}
                    </p>
      ...
      {project && (
        <section className="border-t border-black/[.08] bg-[#fafafa]">
          <div className="mx-auto max-w-6xl border-x border-black/[.06] px-5 py-14 md:px-8 md:py-16">
            <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-center">
              ...
              <Link
                ...
                className="inline-flex h-10 w-fit items-center justify-center gap-2 rounded-md bg-[#1d1d1f] px-4 text-sm font-medium whitespace-nowrap text-white hover:bg-black"
              >
                进入项目
