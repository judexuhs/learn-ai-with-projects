...
colors:
  route-purple: "#5146d8"
  route-purple-hover: "#4036bf"
  route-purple-soft: "#eeecff"
  periwinkle-field: "#f8f8ff"
  ink-blue: "#27304a"
...
components:
  button-primary:
    backgroundColor: "{colors.route-purple}"
    textColor: "{colors.white-paper}"
    rounded: "{rounded.control}"
    padding: "0 20px"
    height: "48px"
  button-primary-hover:
    backgroundColor: "{colors.route-purple-hover}"
    textColor: "{colors.white-paper}"
  button-secondary:
    backgroundColor: "{colors.white-paper}"
    textColor: "{colors.route-purple-hover}"
    rounded: "{rounded.control}"
    padding: "10px 15px"
  navigation-active:
    backgroundColor: "{colors.white-paper}"
...
柔和的长春花紫场景、墨蓝文字和白色纸张卡片构成底层世界；晴黄色胶带、珊瑚色与薄荷色路线标记负责提示进度和分流。轻微旋转、不规则背景色块和环境阴影带来手工纸张感，但信息结构始终稳定、易扫读。

**Key Characteristics:**

- 每个首屏只突出一个推荐动作。
- 大面积浅色底承载内容，紫蓝用于路线、状态和主行动。
- 圆角纸张卡片配合柔和环境阴影，避免硬边工具感。
- 珊瑚与薄荷只标记不同学习方向，不与主紫色争夺行动层级。
- 中文系统字体优先，字重和紧凑字距建立清楚层级，不依赖外部字体资产。

## Colors

色彩以冷静的长春花紫和墨蓝为骨架，用阳光黄、珊瑚和薄荷制造温暖、可辨认的路线标记。

### Primary

- **路线紫** (`#5146d8`)：主按钮、品牌符号、链接、当前路线和关键编号。
- **深路线紫** (`#4036bf`)：主按钮悬停、强调链接及白底控件文字。
- **浅长春花紫** (`#eeecff`)：导航底、标签、步骤编号和低强度选中面。

### Secondary

- **阳光黄** (`#fff0b8`)：胶带、项目提示和深色项目票据中的主行动。
- **珊瑚标记** (`#ff8066`) 与 **珊瑚底** (`#fff0ec`)：第一条分支路线及其低强度容器。
- **薄荷标记** (`#37a477`) 与 **薄荷底** (`#e9f8f1`)：完成状态、第二条分支路线及其低强度容器。
...
- **长春花纸底** (`#f8f8ff`)：consumer 页面主背景。
- **白纸** (`#ffffff`)：导航选中面、路线图和内容卡片。

### Named Rules

**The One Clear Route Rule.** 路线紫只负责当前路径与主行动；珊瑚、薄荷和黄色承担分类或提示，不生成同等级的竞争按钮。
...
- **Label**（650–800，12–14px，1.55）：导航、状态、按钮和路线编号；不强制全大写。项目路径编号继续使用中文 UI 字体栈和路线紫标签样式，不切换为等宽字体。
...
系统采用“浅色分层 + 环境阴影”的混合深度。大区块主要靠背景色分开，阴影集中在白纸卡片、浮层、主按钮和深色项目票据上；阴影颜色偏紫蓝或对应标记色，边缘保持柔和，不使用硬黑描边制造层级。

### Shadow Vocabulary

- **品牌浮标** (`0 8px 20px rgb(81 70 216 / .22)`)：旋转的品牌图标。
- **主行动** (`0 12px 28px rgb(81 70 216 / .24)`)：唯一主按钮。
...
圆角从 10–14px 的控件、15–18px 的图标容器、19–24px 的内容卡片递进到 26–28px 的主特征卡片。品牌图标、路线图、项目符号和项目票据可旋转约 `-4deg` 到 `1.4deg`，形成轻微手作感。圆形只用于路线节点与背景装饰，不作为通用卡片形状。边框保持低对比，导航分隔线可使用淡紫实线，路线图内部使用淡灰紫虚线。
...
- **Primary:** 路线紫底、白字、紫色环境阴影；首页首屏设最小宽度 150px，移动端占满宽度。
- **Hover / Focus:** 悬停变为深路线紫；键盘焦点使用 `3px solid rgb(81 70 216 / .28)` 与 3–4px 外偏移。全局减少动态偏好把过渡压缩到 `0.01ms`。
- **Secondary:** 白底、深路线紫文字、`#d7d4f3` 细边框；悬停时边框加深且背景变为 `#f5f3ff`。
- **Ticket action:** 深墨蓝票据内改用阳光黄底与深棕字，无阴影；悬停为 `#ffe58a`。

### Chips

- **Style:** 标签和说明胶囊使用 10–12px 圆角、浅长春花紫底与路线紫字；保持小号、短文本，不作为主要行动。
- **State:** 导航选中状态切到白纸底和深路线紫字，并增加极浅阴影。

### Cards / Containers

- **Corner Style:** 目标路线 22px，项目票据 26px，学习路线图 28px。
- **Background:** 白纸用于主路线与通用内容；珊瑚底、薄荷底区分两条学习目标；深墨蓝只用于项目票据和页脚。
- **Shadow Strategy:** 主特征卡片有环境阴影；彩色目标卡片静态时无阴影，只在悬停时抬升 2–3px。
- **Border:** 大多数内容卡片无边框；导航与弹出菜单使用低对比淡紫边框。
- **Internal Padding:** 常规内容 20–28px，主路线图桌面为 `26px 26px 22px`，移动端为 `23px 20px 19px`。

### Navigation

桌面头部高 72px，使用半透明长春花纸底、16px 背景模糊和淡紫底边。中央导航放在浅紫色 14px 圆角轨道内，链接为 13px/650；当前页使用白底、深紫字和轻阴影。980px 以下替换为带语义标签的 `details/summary` 菜单，菜单宽 230px；移动首要入口置于菜单底部并使用路线紫。
...
项目列表使用白色 20px 圆角卡片与轻环境阴影。路径编号是 46px、15px 圆角的彩色标签：默认浅长春花紫底与路线紫字，后续路径依次使用珊瑚、薄荷和黄色组合。编号使用当前无衬线 UI 字体与 800 字重，不使用等宽字体。项目页“先体验四个成品 Demo”沿用路线紫主按钮，而不是另设 Demo 行动色。

### Learning Map

这是系统的签名组件：白纸 28px 圆角卡片轻微顺时针旋转，上沿贴一条半透明黄色胶带。三站路线由 3px 浅紫轨道连接，42px 圆形节点依次使用紫、珊瑚和薄荷；节点有 5px 白色套边及同色柔光。13px 路线正文固定使用 `#626a80`，标题、说明和完成图标保持单一路径的阅读顺序。

### Goal Lane

整行作为链接，桌面使用“图标 / 说明 / 行动”三列，移动端变为两列并让行动落到说明下方。珊瑚与薄荷只负责区分学习目标；悬停整体上移 3px 并出现柔和阴影，内部行动保持半透明白底。

## Do's and Don'ts

### Do:

- **Do** 在每个首屏只保留一个最醒目的主行动，并用路线紫标识。
- **Do** 用白纸、长春花紫、珊瑚底和薄荷底建立大面积低对比分区。
...
- **Don't** 同时使用多个路线紫主按钮，或让珊瑚、薄荷成为竞争性的主行动色。
