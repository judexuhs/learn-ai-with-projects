...
test('ships eighteen substantial topics across three learning layers', () => {
  ...
  for (const topic of topics) {
    ...
    assert.ok(
      ...
      `${topic.slug} needs at least four teaching sections`,
    );
    assert.ok(
      topic.sections.every((section) => section.paragraphs.length >= 1),
    );
    assert.ok(
      topic.example?.title.length > 0,
      `${topic.slug} needs a concrete example`,
    );
    assert.ok(
      topic.example?.setup.length > 0,
      `${topic.slug} needs an example setup`,
    );
    assert.ok(
      topic.example?.before.length > 0,
      `${topic.slug} needs an example input`,
    );
    assert.ok(
      topic.example?.after.length > 0,
      `${topic.slug} needs an example result`,
    );
    assert.ok(
      topic.visual?.title.length > 0,
      `${topic.slug} needs an explanatory visual`,
    );
    assert.ok(
      ['flow', 'layers', 'compare', 'bars'].includes(topic.visual?.type),
      `${topic.slug} uses an unsupported visual type`,
    );
    assert.ok(
      topic.visual?.items.length >= 3,
      `${topic.slug} visual needs at least three items`,
    );
    assert.ok(
      topic.experiment.steps.length >= 3,
      `${topic.slug} needs a hands-on experiment`,
    );
...
test('three goal paths only reference existing topics and projects', () => {
  ...
  for (const path of learningPaths) {
    ...
    assert.equal(
      new Set(path.steps.map((step) => step.topic)).size,
      path.steps.length,
    );
  }
});
